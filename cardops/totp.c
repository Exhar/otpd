 ...
 *  c200.c 
 *
 *  A TOTP implementation. It's mostly based on HOTP code, because 
 *  TOTP = HOTP(K, T), where time is UNIX time divided by the time shift.
 *  This implementation has the timeshift set to 30 sec.
 *
...
static struct {
  ...
  { "totp-d6", HOTP6 },	/* 6 digit response */
  { "totp-d7", HOTP7 },	/* 7 digit response */
  { "totp-d8", HOTP8 },	/* 8 digit response */
 ...
 */
static int
totp_name2fm(user_t *user, __unused__ state_t *state)
{
  int i;
 ...
 */
static int
totp_keystring2key(user_t *user, state_t *state)
{
 ...
 */
static int
totp_nullstate(__unused__ const config_t *config,__unused__ const user_t *user,
 ...
 */
static int
totp_challenge(const user_t *user, state_t *state,
 ...
 */
static int
totp_response(const user_t *user, state_t *state,
              ...
              __unused__ size_t len, char response[OTP_MAX_RESPONSE_LEN + 1])
{
  ...
  mlog(LOG_DEBUG1, "totp: iv is set to %lu", iv);
 ...
 */
static int
totp_updatecsd(state_t *state, __unused__ time_t when,
 ...
 */
static int
totp_isconsecutive(const user_t *user, state_t *state, int thisewin)
{
  return 1;	
}
...
static int
totp_maxtwin(__unused__ const user_t *user, __unused__ state_t *state,
...
static char *
totp_printchallenge(char s[OTP_MAX_CHALLENGE_LEN * 2 + 1],
...
/* cardops instance */
static cardops_t totp_cardops = {
  .prefix		= "totp",
  .plen			= 4, /* strlen("hotp") */

  .name2fm		= totp_name2fm,
  .keystring2key	= totp_keystring2key,
  .nullstate		= totp_nullstate,
  .challenge		= totp_challenge,
  .response		= totp_response,
  .updatecsd		= totp_updatecsd,
  .isconsecutive	= totp_isconsecutive,
  .maxtwin		= totp_maxtwin,
  .printchallenge	= totp_printchallenge,
};
...
totp_init(void)
{
  if (ncardops == OTP_MAX_VENDORS) {
    mlog(LOG_CRIT, "totp_init: module limit exceeded");
    exit(1);
  }

  cardops[ncardops++] = totp_cardops;
}
